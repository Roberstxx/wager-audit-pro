  ...
  return (
    <div className="space-y-8">
      <header>
        ...
        <p className="text-sm text-muted-foreground">Estadísticas, drawdown y comparativas para entender tu rentabilidad real.</p>
      </header>

      <section className="grid gap-3 md:grid-cols-3">
        <div className="rounded-xl border border-border bg-card p-5">
          <p className="text-xs uppercase tracking-wider text-muted-foreground">Saldo máximo histórico</p>
          <p className="mt-2 font-display text-2xl font-semibold tabular-nums">{fmt(m.maxBalance)}</p>
        </div>
        <div className="rounded-xl border border-border bg-card p-5">
          <p className="text-xs uppercase tracking-wider text-muted-foreground">Saldo actual</p>
          <p className="mt-2 font-display text-2xl font-semibold tabular-nums">{fmt(m.currentBalance)}</p>
        </div>
        <div className="rounded-xl border border-[color:var(--loss)]/30 bg-[color:var(--loss)]/5 p-5">
          ...
          <p className="mt-2 font-display text-2xl font-semibold tabular-nums text-[color:var(--loss)]">{fmt(m.drawdown)}</p>
          <p className="mt-1 text-xs text-muted-foreground">
            {m.drawdown > 0
              ? `Has devuelto ${fmt(m.drawdown)} (${fmtPct(m.drawdownPct)}) desde tu punto más alto.`
      ...
      <section className="grid gap-4 lg:grid-cols-2">
        <div className="rounded-2xl border border-border bg-card p-5">
          ...
          <div className="h-72">
            {monthly.length === 0 ? (
              <div className="flex h-full items-center justify-center text-sm text-muted-foreground">Sin datos suficientes.</div>
            ) : (
              <ResponsiveContainer>
                <BarChart data={monthly}>
                  ...
                  <Tooltip contentStyle={{ background: "oklch(0.21 0.018 260)", border: "1px solid oklch(0.28 0.02 260)", borderRadius: 8 }} formatter={(v: number) => fmt(v)} />
                  <Legend wrapperStyle={{ fontSize: 12 }} />
                  <Bar dataKey="profit" name="Ganancia" fill="oklch(0.74 0.19 152)" radius={[4, 4, 0, 0]} />
                  <Bar dataKey="loss" name="Pérdida" fill="oklch(0.65 0.24 22)" radius={[4, 4, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            )}
          </div>
              ...
              <ResponsiveContainer>
                <PieChart>
                  <Pie data={distribution} dataKey="value" nameKey="name" innerRadius={60} outerRadius={100} paddingAngle={3}>
                    {distribution.map((d, i) => <Cell key={i} fill={d.color} stroke="transparent" />)}
                  </Pie>
                  <Tooltip contentStyle={{ background: "oklch(0.21 0.018 260)", border: "1px solid oklch(0.28 0.02 260)", borderRadius: 8 }} />
                  <Legend wrapperStyle={{ fontSize: 12 }} />
                </PieChart>
              </ResponsiveContainer>
            )}
          ...
          <div className="grid grid-cols-2 gap-3 text-sm">
            {[
              ...
              <div key={label} className="rounded-xl border border-border bg-background/40 p-4">
                ...
                <p className="mt-2 font-display text-xl font-semibold tabular-nums">{s.total} apuestas</p>
                <dl className="mt-3 space-y-1.5 text-xs">
                  <div className="flex justify-between"><dt className="text-muted-foreground">Ganancia</dt><dd className="text-[color:var(--profit)] tabular-nums">{fmt(s.profit)}</dd></div>
                  <div className="flex justify-between"><dt className="text-muted-foreground">Pérdida</dt><dd className="text-[color:var(--loss)] tabular-nums">{fmt(s.loss)}</dd></div>
                  <div className="flex justify-between"><dt className="text-muted-foreground">Neto</dt><dd className={`tabular-nums ${s.net >= 0 ? "text-[color:var(--profit)]" : "text-[color:var(--loss)]"}`}>{fmt(s.net)}</dd></div>
                  <div className="flex justify-between"><dt className="text-muted-foreground">Win rate</dt><dd className="tabular-nums">{fmtPct(s.winRate)}</dd></div>
                </dl>
              </div>
            ))}
          </div>
      ...
      <p className="text-center text-xs text-muted-foreground">
        Esta herramienta es solo para análisis financiero personal. No emite recomendaciones ni predicciones deportivas.
      </p>
    </div>
  );
}
